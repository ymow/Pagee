...
export default function TxtViewer({ data, onToggleChat, onSelectionAction, onChapterChange }: TxtViewerProps) {
  ...
  useEffect(() => {
    ...
    // For txt files, we just treat the whole text as one chapter for simplicity in MVP
    onChapterChange?.("全文內容", decodedText.substring(0, 5000));
  ...
  return (
    <div className="flex flex-col h-screen bg-white relative" onMouseUp={handleMouseUp}>
      ...
      <main className="flex-1 overflow-y-auto bg-stone-100/30 flex justify-center p-8 custom-scrollbar">
        <div className="max-w-3xl w-full bg-white shadow-sm border rounded-xl p-10 leading-relaxed text-stone-800 whitespace-pre-wrap font-sans">
          {text || "載入中..."}
        </div>
      </main>

      <footer className="px-6 py-2 border-t bg-stone-50/50 text-xs text-stone-400 flex justify-between items-center">
        <span>Pagee Text Reader</span>
        <span>100%</span>
      </footer>
